...
function [ ] = fn_Main( )
    ...
    X_pre_estimate = zeros(21,length(Time)+1);
    X_post_estimate = zeros(23,length(Time)+1);
    X_post_estimate(:,1,1) = X_a_0;
    ...
    P_post(4:6,4:6) = 0.5*eye(3,3);
    P_post(7:9,7:9) = eye(3,3);   
    P_post(10:12,10:12) = 1000*eye(3,3);
    P_post(13:15,13:15) = eye(3,3);
    P_post(16:18,16:18) = eye(3,3);
    P_post(19:21,19:21) = 0.25*eye(3,3);
    ...
    for iCount = 2:length(v_time)
        ...
        %q_nominal = q_nominal/quatnorm(q_nominal');
%         del_q = fn_CrossTensor(X_a_init(1:4,end),0)*[-q_nominal(1:3);q_nominal(4)];
%         X_pre(1:3,end) = del_q(1:3);   
        
        %Run the dynamic model to get true states
        time = v_time(iCount);
        [T,X] = ode45(@fn_StateSpace,[v_time(iCount-1),v_time(iCount)],X_a_itr,options);
        X_a_pre = [X_a_pre,X(end,:)'];
        Total_T = [Total_T;T(end)'];
        X_a_init = X_a_pre(:,end);
        ...
        %state propagation               
        Phi = fn_Create_Phi(X_a_init(:,end), n, t_delta);
        Q_r = fn_Create_Q_r(Phi,X_a_init(:,end), t_delta, sig_tau,sig_p);
        ...
        h = fn_Create_h(X_a_init(1:4,end),X_pre); 
        %Fetch measurement
        signal(1:3,1) = Signal_vector.Data(:,:,iCount-1); 
        q_measured = Signal_Quaternion(iCount-1,:)';
...
%        X_pre = [X_pre, Phi*X_pre(:,end)];
        X_a_test = [X_a_test,X_a_itr];
        P_post = (eye(21,21)-K*Hk)*P_pre;
  
    end
    toc;
    %Generate measurements to compare with actual measurements
    Mu_est = zeros(4,length(X_a_test));
    r_c_est = zeros(3,length(X_a_test));
    for iCount = 1:length(X_a_test)
       Mu_est(:,iCount) = fn_CrossTensor(X_a_test(20:23,iCount),0)*X_a_test(1:4,iCount);
       r_c_est(1:3,iCount) = X_a_test(11:13,iCount) + rho_c + fn_CreateRotationMatrix(X_a_test(1:4,iCount))*X_a_test(17:19)';
    end
    ...
    figure;
    subplot(2,1,1);
    plot(dy_time,X_a_test(18:20,:)');hold all;
    plot(dy_time,X_a(:,18:20),'LineStyle','-.');
    legend('\rho_tx_est','\rho_ty_est','\rho_tz_est','\rho_tx_true','\rho_ty_true','\rho_tz_true');
    ylabel('\rho_t');
    subplot(2,1,2);
    plot(dy_time,X_a_test(20:23,:)');hold all;
...
function[dy] = fn_StateSpace(~,X_a)
    ...
    psi = [p_x*omega(2)*omega(3); p_y*omega(1)*omega(3);p_z*omega(1)*omega(2)];
    q_dot = 0.5*fn_CrossTensor(q_omega_rel,0)*q;
    tau = [tau_1;tau_2;tau_3];
    e_force = [e_force_x;e_force_y;e_force_z];
    omega_dot = psi + J*tau.*rand(3,1); %for static conditions
    %omega_dot = zeros(3,1);
    p_dot = zeros(3,1);
    rho_t_dot = zeros(3,1);
    ita_dot = zeros(4,1);
...
%Create state propagation matrix for the rotation component
...
%#codegen
    ...
    del_Ita_k = fn_VectorToSkewSymmetricTensor(del_ita_k);
    first = -del_Ita_k + del_ita_k(4)*eye(3,3);
    del_Q_k = fn_VectorToSkewSymmetricTensor(del_q_k) ;
